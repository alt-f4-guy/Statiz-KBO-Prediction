...
# 🛡️ 최적화 파라미터 로드 시스템
# ---------------------------------------------------------
PARAMS_FILE = "Option_A/best_hyperparameters.csv"
DEFAULT_PARAMS = {
        ...
        while True:
            ...
            for g in pending_games:
                ...
                if not X_feat.empty:
                    ...
                    mu_h = (ensemble_models['homeScore']['cb'].predict(X_test)[0] * w_cb) + (ensemble_models['homeScore']['rf'].predict(X_test_rf)[0] * w_rf) + (ensemble_models['homeScore']['lgb'].predict(X_test)[0] * w_lgb) + (ensemble_models['homeScore']['svr'].predict(X_test_ridge_sc)[0] * w_svr) + (ensemble_models['homeScore']['ridge'].predict(X_test_ridge_sc)[0] * w_rd)
                    mu_a = (ensemble_models['awayScore']['cb'].predict(X_test)[0] * w_cb) + (ensemble_models['awayScore']['rf'].predict(X_test_rf)[0] * w_rf) + (ensemble_models['awayScore']['lgb'].predict(X_test)[0] * w_lgb) + (ensemble_models['awayScore']['svr'].predict(X_test_ridge_sc)[0] * w_svr) + (ensemble_models['awayScore']['ridge'].predict(X_test_ridge_sc)[0] * w_rd)
