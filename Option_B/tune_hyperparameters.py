...
def objective(trial):
    ...
    mlp_params = {
        ...
        "alpha": trial.suggest_float("mlp_alpha", 0.01, 10.0, log=True),
    ...
    w_mlp = trial.suggest_float("ens_mlp", 0.05, 0.3)
    ...
    for m in months:
        ...
        tw = w_cb + w_rf + w_lgb + w_mlp + w_rd
        nw = (w_cb/tw, w_rf/tw, w_lgb/tw, w_mlp/tw, w_rd/tw)
        ...
        for target in ['homeScore', 'awayScore']:
            ...
            m_mlp = MLPRegressor(**mlp_params).fit(X_tr_rd, y_tr) # MLPRegressor does not support sample_weight directly
            p_mlp = m_mlp.predict(X_ts_rd)
            ...
            mu_preds[key] = (p_cb * nw[0]) + (p_rf * nw[1]) + (p_lgb * nw[2]) + (p_mlp * nw[3]) + (p_rd * nw[4])
            del m_cb, m_rf, m_lgb, m_mlp, m_rd
...
if __name__ == "__main__":
    ...
    pd.DataFrame([study.best_params]).to_csv("Option_B/best_hyperparameters.csv", index=False)
    console.print("\n📝 [bold yellow]최적 파라미터 저장 완료: Option_B/best_hyperparameters.csv[/bold yellow]")
