...
# Statiz API 설정
API_KEY = "245281b86d65c6edba076ff86cc7a16d"
SECRET  = "794b143fe19e273aa3f2e2015ce7352ec6e681537f2345f8237d88174a3b33c8"
PTT_IDX = "05"
...
def get_now(): return datetime.now().strftime('%H:%M:%S')
...
def run_realtime_prediction_system():
    console.clear()
    console.print(Panel.fit("[bold cyan]🚀 KBO Infinite Watchdog v10.0[/bold cyan]", border_style="cyan"))
    ...
    # 1. 앙상블 모델 사전 학습
    with Status("[bold magenta]🧠 4종 앙상블 모델 최적화 학습 중...", console=console) as status:
        try:
            ...
            for col in ['homeTeam', 'awayTeam']: train_df[col] = train_df[col].map(team_map)
    ...
    # 가중치 정규화
    total_w = P['ens_cb'] + P['ens_rf'] + P['ens_lgb'] + P['ens_mlp'] + P['ens_rd']
    w_cb, w_rf, w_lgb, w_mlp, w_rd = P['ens_cb']/total_w, P['ens_rf']/total_w, P['ens_lgb']/total_w, P['ens_mlp']/total_w, P['ens_rd']/total_w
    ...
    with Live(Panel(results_table, title=f"📡 {today_str} 라인업 실시간 감시 중...", border_style="blue"), refresh_per_second=1) as live:
        while True:
            ...
            for g in pending_games:
                ...
                all_players = []
                if isinstance(l_data, list): all_players = l_data
                elif isinstance(l_data, dict):
                    for v in l_data.values():
                        if isinstance(v, list): all_players.extend(v)
                ...
                if not X_feat.empty:
                    ...
                    for col in ['homeTeam', 'awayTeam']: X_test[col] = X_test[col].map(team_map)
                    ...
                    mu_h = (ensemble_models['homeScore']['cb'].predict(X_test)[0] * w_cb) + (ensemble_models['homeScore']['rf'].predict(X_test_rf)[0] * w_rf) + (ensemble_models['homeScore']['lgb'].predict(X_test)[0] * w_lgb) + (ensemble_models['homeScore']['mlp'].predict(X_test_ridge_sc)[0] * w_mlp) + (ensemble_models['homeScore']['ridge'].predict(X_test_ridge_sc)[0] * w_rd)
                    mu_a = (ensemble_models['awayScore']['cb'].predict(X_test)[0] * w_cb) + (ensemble_models['awayScore']['rf'].predict(X_test_rf)[0] * w_rf) + (ensemble_models['awayScore']['lgb'].predict(X_test)[0] * w_lgb) + (ensemble_models['awayScore']['mlp'].predict(X_test_ridge_sc)[0] * w_mlp) + (ensemble_models['awayScore']['ridge'].predict(X_test_ridge_sc)[0] * w_rd)
                    
                    prob = calculate_win_prob(mu_h, mu_a)
                    win_team = h_name if prob >= 0.5 else a_name
                    
                    payload = {"ptt_idx": PTT_IDX, "s_no": s_no, "homeTeam": h_name, "awayTeam": a_name, "predictWinTeam": win_team, "percent": round(float(prob) * 100, 2), "update_time": datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
