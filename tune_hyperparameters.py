...
def calculate_win_prob(h_mu, a_mu):
    """
    홈팀과 어웨이팀의 득점 기댓값을 바탕으로 Skellam 분포를 사용하여 승리 확률 계산
    """
...
def objective(trial):
    """
    Optuna 파라미터 튜닝 목적 함수 (Option A: SVR 도입 5종 앙상블 조합)
    """
    # 연도별 데이터 가중치 튜닝
    weight_map = {
    ...
    # 3. SVR (Support Vector Regressor) 하이퍼파라미터
    svr_params = {
    ...
    # 월별 순회하며 Walk-forward 검증 수행 (학습 리소스를 줄이기 위해 월 단위 학습 적용)
    for m in months:
...
def make_dashboard(study):
    """
    Optuna 튜닝 실시간 모니터링을 위한 대시보드 UI 생성
    """
